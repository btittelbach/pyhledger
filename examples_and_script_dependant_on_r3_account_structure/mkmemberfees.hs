{-# LANGUAGE OverloadedStrings,Arrows,BangPatterns,FlexibleContexts #-}
...
import Data.Maybe
import Data.Decimal
--import Data.Text hiding (map,foldl1,foldl,filter)
import Data.Text (pack,unpack,empty,singleton)
...
euro n = Amount{acommodity="EUR", aquantity=(realFracToDecimal 4 n), astyle=amountstyle{asprecision=Precision 2}, aprice=Nothing}
...
mkMonthlyMembershipFeeTransactions cur_year cur_month xs = [ Transaction {
    ...
    tcode="",
    tdescription=pack $ "Membership Fee due for "++pname,
    tcomment=pack $ (show month),
    ttags=[],
    tpostings=[
      posting {paccount=pack ("revenue:membership-fees"), pamount=mixed [euro ((*) (-1.0) fee)]}
      ,posting {paccount=pack ("assets:current:membership A/R:"++pname), pamount=missingmixedamt}
    ],
    tprecedingcomment="Autogenerated from sqlite database"
    } | month <- months ]
        where
            pname = (fromSql $ xs !! 0) :: String
            firstmonthyear = (fromSql $ xs !! 1) :: Integer
...
mkMonthlyRecurringInternalTransfers cur_year cur_month (results, sqlpostings) = [ Transaction {
    ...
    tsourcepos=nullsourcepos,    
    tcode=pack $ "",
    tdescription=pack (fromSql $ results !! 0),
    tcomment=pack (show month),
    ttags=[],
    tpostings=[posting {paccount=(fromSql acct),pamount=mixed[euro ((fromRational $ fromSql amnt):: Decimal)]} | acct:amnt:curr:xl <- sqlpostings],
    tprecedingcomment="Autogenerated from sqlite database"
    } | month <- months ]
        where
...
main = do
    ...
    mapM_ (T.putStrLn . showTransaction) $ sortBy (comparing tdate) $ ((foldl (++) [] (map (mkMonthlyMembershipFeeTransactions year month) membershipfees))::[Transaction]) ++ ((foldl (++) [] (map (mkMonthlyRecurringInternalTransfers year month) (filter filterValidPostingsList otherrecurringtransfers_plus_postings)))::[Transaction])
    disconnect conn
